...
const SectionWithAnimation = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div ref={ref} className={`overflow-hidden transition-h-o-m ease-spring duration-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      {children}
    </div>
  )
}
...
export default function Page() {
  ...
  const Section = ({ id, title, children, action, icon }:
    ...
    <section id={id} className="scroll-mt-24">
      <SectionWithAnimation>
        <div className="card p-6 md:p-8 hover:scale-[1.02] hover:shadow-xl overflow-hidden transition-h-o-m ease-spring duration-600">
  ...
  */
  ...
  const PdfCard = ({
    ...
    return (
      <div className="file-card rounded-2xl border border-neutral-200 dark:border-neutral-800 p-4 md:p-5 shadow-md hover:shadow-lg overflow-hidden transition-h-o-m ease-spring duration-600">
  ...
  const SkillPill = ({ name, description }: { name: string, description: string }) => {
    ...
    return (
      <div className="relative">
        <span
          className="inline-flex items-center rounded-full px-3 py-1 text-sm font-medium cursor-pointer overflow-hidden transition-h-o-m ease-spring duration-600 hover:scale-105
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[var(--bg)] text-[var(--ink)]">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-10 md:pt-14 pb-6 md:pb-10 relative z-10">
        <SectionWithAnimation>
          <div className="card overflow-hidden rounded2xl p-6 md:p-8 hover:scale-[1.02] hover:shadow-xl overflow-hidden transition-h-o-m ease-spring duration-600">
        ...
        <Section id="quick-stats" title="Quick Stats" icon={<BarChart2 className="h-6 w-6 text-[var(--primary-orange)]" />}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {content.quickStats.map((stat, index) => (
              <div key={index} className="bg-neutral-50 dark:bg-[var(--bg)] p-5 rounded-xl shadow-sm hover:scale-[1.05] hover:shadow-lg overflow-hidden transition-h-o-m ease-spring duration-600">
